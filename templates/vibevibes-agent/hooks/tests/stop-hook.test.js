...
test("blocks exit with idle message when no events", () => {
  const result = makeDecision({
    events: [],
  ...
  assert.equal(result.decision, "block");
  assert.ok(
    result.reason.toLowerCase().includes("quiet") ||
      result.reason.toLowerCase().includes("watching") ||
      result.reason.toLowerCase().includes("idle")
  );
});
...
test("formats idle state when no events", () => {
  const prompt = formatPrompt({
    events: [],
  ...
  assert.ok(
    prompt.toLowerCase().includes("quiet") ||
      prompt.toLowerCase().includes("idle") ||
      prompt.toLowerCase().includes("watching") ||
      prompt.toLowerCase().includes("no activity")
  );
});
